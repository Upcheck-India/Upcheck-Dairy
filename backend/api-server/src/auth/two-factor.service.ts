...
export class TwoFactorService {
  ...
  private async getUser(userId: string) {
    const [user] = await db.select().from(farmers).where(eq(farmers.id, userId));
  ...
  async isEnabled(userId: string): Promise<boolean> {
    const [user] = await db.select({ is2faEnabled: farmers.is2faEnabled }).from(farmers).where(eq(farmers.id, userId));
  ...
  async verifyCode(userId: string, token: string): Promise<boolean> {
    const [user] = await db.select({ totpSecret: farmers.totpSecret, is2faEnabled: farmers.is2faEnabled }).from(farmers).where(eq(farmers.id, userId));
